...
from schema.order import CartItem, Order, Payment
...
async def get_order_history(user: user_dependency, db: db_dependency):
    orders = db.query(Orders).options(joinedload(Orders.items)).filter(Orders.username == user['username']).all()   
...
async def notify_user(order_id: str):
    print(f"User notification: User Order {order_id} has been confirm and is being processed")
