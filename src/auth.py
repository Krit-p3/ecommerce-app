...
user_oauth2_bearer = OAuth2PasswordBearer(
    tokenUrl="/user/token", scheme_name="user_oauth2_bearer"
)
admin_oauth2_bearer = OAuth2PasswordBearer(
    tokenUrl="/admin/token", scheme_name="admin_oauth2_bearer"
)
...
def create_user(username: str, password: str, role: str):
    if role == "users":
        user = Users(username=username, hashed_password=bcrypt_context.hash(password))
        return user
    elif role == "admin":
        admin = Admin(username=username, hashed_password=bcrypt_context.hash(password))
        return admin
    else:
        return None
